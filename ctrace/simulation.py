...
class InfectionState:
    def __init__(self, G: nx.graph, SIR: SIR_Tuple, budget: int, policy: str, transmission_rate: float, transmission_known: bool = False, compliance_rate: float = 1, compliance_known: bool = False, discovery_rate: float = 1, snitch_rate: float = 1):
        ...
        # the compliance_rate average for each label demographic
        ...
        mean_duration = np.mean(
            list(nx.get_edge_attributes(G, "duration").values()))
        lambda_cdf = -np.log(1-transmission_rate)/mean_duration
        def exponential_cdf(x): return 1-np.exp(-lambda_cdf*x)
        ...
        self.compliance_map = [(1-k*(1-self.compliance_map[i]))
                               for i in range(len(self.compliance_map))]
    ...
    def step(self, quarantine: Set[int]):
        ...
        full_data = EoN.discrete_SIR(G=self.G, test_transmission=edge_transmission, args=(
            self.G,), initial_infecteds=self.SIR.I1 + self.SIR.I2, initial_recovereds=self.SIR.R, tmin=0, tmax=1, return_full_data=True)

        S = [k for (k, v) in full_data.get_statuses(
            time=1).items() if v == 'S']
        I1 = [k for (k, v) in full_data.get_statuses(
            time=1).items() if v == 'I']
        I2 = self.SIR.I1
        ...
        for node in self.G.nodes:
            ...
            self.G.nodes[node]['quarantine'] = 2 if node in quarantine and random.random(
            ) < self.G.nodes[node]['compliance_rate'] else max(self.G.nodes[node]['quarantine'], 0)

        self.set_contours()

    def set_contours(self):
        # For knowledge of which edges are complied along, add parameter compliance_known:bool
        (self.V1, self.V2, self.P, self.Q) = find_excluded_contours_edges_PQ(
