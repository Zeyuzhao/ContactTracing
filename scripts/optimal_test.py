    ...
    for v in state.V2:
        ...
        for u in (set(state.G.neighbors(v)) & state.V1):
            if u not in quarantine:
                ...
                expected *= (1-(1-state.G.nodes[u]
                             ['compliance_rate'])*P[u]*Q[u][v])

        if v in quarantine:
            total += (1-P[v])*(1-expected) * \
                (1-state.G.nodes[v]['compliance_rate'])
        else:
            total += (1-P[v])*(1-expected)
    return total
...
out_schema = ["infection_size", "V1_size", "V2_size", "edge_size", "D", "ip_expect",
              "dep_expect", "deg_expect", "time_ip", "time_dep", "time_deg", "ratio_dep", "ratio_deg"]
...
def optimal_ratio(G: nx.graph, budget: int, policy: str, transmission_rate: float, transmission_known: bool, compliance_rate: float, compliance_known: bool, discovery_rate: float, snitch_rate: float, from_cache: str, **kwargs):
    ...
    quarantine = set([optimal_obj.quarantine_map[k]
...
run = GridExecutorParallel.init_multiple(
    config, in_schema, out_schema, func=optimal_ratio, trials=1)
run.exec()
