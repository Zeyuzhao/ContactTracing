...
config = {
    ...
    "discovery_rate": [1],
    "snitch_rate":  [1],
    "from_cache": [i for i in list(os.listdir(PROJECT_ROOT/"data"/"SIR_Cache"/"optimal_trials"/"montgomery_trials")) if i[0]=="m" and i[1] = "a"],
...
def optimal_ratio(G: nx.graph, budget: int, policy: str, transmission_rate: float, tansmission_known: bool, compliance_rate: float, compliance_known: bool, discovery_rate: float, snitch_rate: float, from_cache: str, **kwargs):
    ...
    state = InfectionState(G, (S, I1, I2, R), budget, policy, transmission_rate, trasnmission_known, compliance_rate, compliance_known, discovery_rate, snitch_rate)
